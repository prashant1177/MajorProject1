...
exports.createBooking = async (req, res) => {
  try {
    ...
    const totalPrice = Number(hotel.price * totalDays);
...
exports.getBooked = async (req, res) => {
  const { id } = req.params;
  try { const hotels = await Listing.find({ owner: id }).select('_id'); // Get only the hotel IDs
...
exports.getBookingDetails = async (req, res) => {
  try {
    const bookings = await Booking.findById(req.params.id)
      .populate("hotel")
      .populate("user");
    res.render("./booking/bookingdetails.ejs", { bookings });
  } catch (err) {
    res.status(500).send("Server Error");
  }
};
